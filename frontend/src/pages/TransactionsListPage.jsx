...
import { FiEdit, FiTrash2, FiRotateCcw, FiFilter, FiCalendar, FiDollarSign, FiCreditCard, FiAlertTriangle, FiPlusCircle, FiRepeat, FiCheckSquare, FiSquare, FiRefreshCw } from 'react-icons/fi';
import AddTransactionModal from '../components/transactions/AddTransactionModal'; // Import the modal
...
const fetchTransactions = async ({ queryKey }) => {
  // const [_key, filters] = queryKey; // Example for future filters
  // console.log("Fetching transactions with filters:", filters);
  
  // TODO: When backend supports filtering, pass filters to API calls
  // For now, fetching all and then client-side filtering if needed (less ideal for large datasets)
  const [incomesRes, expensesRes] = await Promise.all([
    apiClient.get('/incomes'), 
    apiClient.get('/expenses?includePlanned=true') // Example: Add a query param to fetch planned too
  ]);
  ...
  // Apply client-side filtering (example - replace with server-side if possible)
  // if (filters.type && filters.type !== 'all') {
  //   allTransactions = allTransactions.filter(tx => tx.type === filters.type);
  // }
  // if (filters.dateFrom) {
  //   allTransactions = allTransactions.filter(tx => parseISO(tx.transactionDate) >= parseISO(filters.dateFrom));
  // }
  // if (filters.dateTo) {
  //   allTransactions = allTransactions.filter(tx => parseISO(tx.transactionDate) <= parseISO(filters.dateTo));
  // }
...
function TransactionsListPage() {
  ...
  // Basic filter state examples
  const [filterType, setFilterType] = useState('all'); // 'all', 'income', 'expense'
  const [showPlanned, setShowPlanned] = useState(true); // To show/hide expenses with isProcessed=false

  // Prepare query key and function with filters
  // Note: React Query will refetch if the queryKey changes
  const queryKey = useMemo(() => ['transactions', { filterType, showPlanned }], [filterType, showPlanned]);

  const { data: rawTransactions, isLoading, error, refetch } = useQuery({
    queryKey: queryKey,
    queryFn: fetchTransactions, // fetchTransactions will need to be adapted to use filters from queryKey
  });

  // Client-side filtering based on state (can be moved to fetchTransactions if API supports it)
  const transactions = useMemo(() => {
    ...
    return filtered;
  }, [rawTransactions, filterType, showPlanned]);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKey }); // Use the dynamic queryKey
      queryClient.invalidateQueries({ queryKey: ['dashboardSummary'] });
      queryClient.invalidateQueries({ queryKey: ['recentTransactions'] });
      queryClient.invalidateQueries({ queryKey: ['expenseDistribution'] });
    },
    onError: (error) => console.error(`Error deleting transaction:`, error)
  });
  ...
  const restoreMutation = useMutation({
      ...
      onError: (error) => console.error('Error restoring transaction:', error)
  });

  const handleDelete = (type, id) => {
    if (window.confirm(`האם אתה בטוח שברצונך למחוק ${type === 'income' ? 'הכנסה' : 'הוצאה'} זו? הפעולה תסמן אותה כמאורכבת.`)) {
  ...
  const getTransactionDisplayDetails = (transaction) => {
    if (transaction.type === 'income') {
      return {
        categoryDisplay: transaction.category?.name || 'ללא קטגוריה',
        icon: <FiDollarSign className="text-green-500" />
      };
    } else { // expense
      let description = transaction.description || transaction.subcategory?.name || 'ללא קטגוריה';
  ...
  if (error) return ( /* ... Error display ... */ <div className="flex flex-col items-center justify-center h-64 text-red-600 bg-red-50 p-4 rounded-md"><FiAlertTriangle className="h-12 w-12 mb-4"/><p className="text-xl font-semibold">שגיאה בטעינת הפעולות</p><p>{error.message}</p><button onClick={() => refetch()} className="mt-4 px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">נסה שוב</button></div>);
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
            {/* Filter Controls */}
            <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="p-2 border border-slate-300 rounded-md bg-white text-sm shadow-sm focus:ring-sky-500 focus:border-sky-500">
                <option value="all">הכל</option>
                <option value="income">הכנסות</option>
                <option value="expense">הוצאות</option>
            </select>
            <label className="flex items-center text-sm text-slate-700 cursor-pointer">
                <input type="checkbox" checked={showPlanned} onChange={(e) => setShowPlanned(e.target.checked)} className="form-checkbox h-4 w-4 text-sky-600 border-slate-300 rounded mr-1 focus:ring-sky-500" />
                הצג פעולות מתוכננות
            </label>
          ...
          <tbody>
            ...
            {!isLoading && transactions && transactions.length > 0 ? (
              transactions.map((tx) => {
                ...
                return (
                  <tr key={`${tx.type}-${tx.id}`} className={`hover:bg-slate-50 transition-colors ${isDeleted ? 'opacity-60 bg-slate-50' : ''}`}>
                    {/* ... (td elements כמו קודם, עם שינויים קלים) ... */}
                    <td className="px-5 py-4 border-b border-slate-200 text-sm"><p className="text-slate-900 whitespace-no-wrap">{tx.date ? format(parseISO(tx.date), 'dd/MM/yy', { locale: he }) : '-'}</p></td>
                    <td className="px-5 py-4 border-b border-slate-200 text-sm"><div className="flex items-center"><div className="mr-3 p-1.5 bg-slate-100 rounded-full shrink-0">{displayDetails.icon}</div><p className="text-slate-900 whitespace-no-wrap truncate max-w-xs" title={tx.description || displayDetails.categoryDisplay}>{tx.description || displayDetails.categoryDisplay}</p></div></td>
                    <td className="px-5 py-4 border-b border-slate-200 text-sm"><span className={`px-2 py-1 text-xs font-semibold leading-tight rounded-full ${tx.type === 'income' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{tx.type === 'income' ? 'הכנסה' : 'הוצאה'}</span></td>
                    <td className={`px-5 py-4 border-b border-slate-200 text-sm font-semibold ${tx.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>{formatCurrency(tx.amount)}</td>
                    <td className="px-5 py-4 border-b border-slate-200 text-sm"><span className={`text-xs px-2 py-0.5 rounded-full ${isDeleted ? 'bg-gray-300 text-gray-700' : (tx.isProcessed ? 'bg-slate-200 text-slate-600' : 'bg-sky-100 text-sky-700')}`}>{isDeleted ? 'מאורכב' : (tx.isProcessed ? 'בוצע' : 'מתוכנן')}</span></td>
                    <td className="px-5 py-4 border-b border-slate-200 text-sm"><div className="flex items-center space-x-1 space-x-reverse">{!isDeleted ? (<button onClick={() => openEditModal(tx)} title="ערוך" className="text-slate-500 hover:text-sky-600 p-1.5 rounded-md hover:bg-sky-50"><FiEdit className="w-4 h-4"/></button>) : null}{!isDeleted ? (<button onClick={() => handleDelete(tx.type, tx.id)} title="מחק" className="text-slate-500 hover:text-red-600 p-1.5 rounded-md hover:bg-red-50" disabled={deleteMutation.isLoading && deleteMutation.variables?.id === tx.id}><FiTrash2 className="w-4 h-4"/></button>) : null}{isDeleted && (<button onClick={() => handleRestore(tx.type, tx.id)} title="שחזר" className="text-slate-500 hover:text-green-600 p-1.5 rounded-md hover:bg-green-50" disabled={restoreMutation.isLoading && restoreMutation.variables?.id === tx.id}><FiRotateCcw className="w-4 h-4"/></button>)}</div></td>
                  </tr>
                );
            })) : (
              !isLoading && ( <tr> <td colSpan="6" className="text-center py-12 text-slate-500"> <FiRepeat className="mx-auto h-12 w-12 text-slate-400 mb-2"/> אין פעולות להצגה. <br/> נסה להוסיף פעולה חדשה או לשנות את המסננים. </td> </tr> )
            )}
          </tbody>
        </table>
      </div>
